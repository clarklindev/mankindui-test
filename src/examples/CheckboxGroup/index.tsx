...
import { Checkbox, Label, Layout, Heading, TabbedCard } from "@mankindui/core";
...
const CheckboxExample = () => {
  ...
  const [savedDataC, updateSavedDataC] = useState(
    Array(options.length).fill(false)
  );
  ...
  return (
    <TabbedCard componentName="Checkbox Group">
      <TabbedCard.Preview>
        <>
          <Layout>
            ...
            <div className="flex flex-row space-x-5">
              {options.map((each, index) => {
                return (
                  <Label
                    key={"CheckboxGroup_CheckboxA" + index}
                    label={each.label}
          ...
          <Layout>
            ...
            <div className="flex flex-col space-y-5">
              {options.map((each, index) => {
                return (
                  <Label
                    key={"CheckboxGroup_CheckboxB" + index}
                    label={each.label}
            ...
            <div className="flex flex-col items-start space-y-10">
              <Label label="hi" labelDirection="right" gap="2">
                <Checkbox
                  ...
                  indeterminate={
                    !savedDataC.every((item) => item === true) &&
                    savedDataC.some((item) => item === true)
                  }
                  name="checkbox"
                  onChange={(event) => {
                    //update group
                    updateSavedDataC(
                      savedDataC.slice().fill(event.target.checked)
                    );
                  }}
                />
              </Label>
              ...
              <div className="flex flex-col spacing-y-5">
                {options.map((each, index) => {
                  return (
                    <Label
                      key={"CheckboxGroup_CheckboxC" + index}
                      label={each.label}
                      labelDirection="right"
                      gap="2"
                    >
                      <Checkbox
                        ...
                        name="Checkbox_ABC"
                        onChange={() =>
                          onChange(savedDataC, updateSavedDataC, index)
                        }
                      />
                    </Label>
                  );
